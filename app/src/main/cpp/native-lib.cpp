 ...
 */
JNIEXPORT jint JNICALL Java_com_example_ffmpegstudydemo_FFmpegHandler_init
        (JNIEnv *jniEnv, jobject instance, jstring url) {

    const char *out_url = (jniEnv)->GetStringUTFChars( url, 0);
    //计算yuv数据的长度
    yuv_width = width;
    yuv_height = height;
...
extern "C" JNIEXPORT jint JNICALL Java_com_example_ffmpegstudydemo_FFmpegHandler_pushCameraData
        (JNIEnv *jniEnv, jobject instance, jbyteArray yArray, jint yLen, jbyteArray uArray, jint uLen,
         jbyteArray vArray, jint vLen) {
    jbyte *yin = (jniEnv)->GetByteArrayElements( yArray, NULL);
    jbyte *uin = (jniEnv)->GetByteArrayElements(uArray, NULL);
    jbyte *vin = (jniEnv)->GetByteArrayElements(vArray, NULL);
...
//                        (long long) ((av_gettime() - startTime) / 1000));
    ...
    av_frame_free(&pFrameYUV);
    av_free(buffers);
    (jniEnv)->ReleaseByteArrayElements(yArray, yin, 0);
    (jniEnv)->ReleaseByteArrayElements(uArray, uin, 0);
    (jniEnv)->ReleaseByteArrayElements(vArray, vin, 0);
 ...
 * 释放资源
 */
extern "C" JNIEXPORT jint JNICALL Java_com_example_ffmpegstudydemo_FFmpegHandler_close
